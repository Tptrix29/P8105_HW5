...
``` r
pth = "./datasets/data/"
data_files = list.files(pth)
extract_info = function(filename, pth){
        tibble(
                read_csv(str_c(pth, filename))
        )
}
longitudial_data = tibble(
        id = str_extract(data_files, pattern = "\\d+"),
        arm = str_extract(data_files, pattern = "^[a-zA-Z]+"),
        data = map(data_files, extract_info, pth)
        ) |> unnest(data)
```
...
longitudial_data |> 
        ...
        mutate(label = str_c(id, arm, sep = "_"), week = as.numeric(week)) |>
...
![](p8105_hw5_pt2632_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
